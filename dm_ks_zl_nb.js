...
循环助力
cookie变量KS_COOKIE
化肥助力码变量NBHFKS_SHARECODE
水滴助力码变量NBHFKS_SHARECODE
[Script]
cron "3 0 * * *" script-path=https://github.com/daimafengzi/siku.git, tag=快手内部助力
*/
const $ = new Env('快手内部助力')
let cookieArr = [];
let NBHFKS_SHARECODEArr = [], NBSDKS_SHARECODEArr = [],message, allMessage = '';
if ($.isNode()) {
...
//获取化肥助力码
if ($.isNode()) {
  if (process.env.NBHFKS_SHARECODE && process.env.NBHFKS_SHARECODE.indexOf('\n') > -1) {
      NBHFKS_SHARECODEs = process.env.NBHFKS_SHARECODE.split('\n');
  } else {
      NBHFKS_SHARECODEs = process.env.NBHFKS_SHARECODE.split()
  };
  Object.keys(NBHFKS_SHARECODEs).forEach((item) => {
        if (NBHFKS_SHARECODEs[item]) {
          NBHFKS_SHARECODEArr.push(NBHFKS_SHARECODEs[item])
        }
      })
} else {
   NBHFKS_SHARECODEArr.push($.getdata('NBHFKS_SHARECODE'));
   $.getjson('NBHFKS_SHARECODE', []).forEach(sharecode => NBHFKS_SHARECODEArr.push(sharecode));
}
//获取化肥助力码结束
//获取水滴助力码
if ($.isNode()) {
  if (process.env.NBSDKS_SHARECODE && process.env.NBSDKS_SHARECODE.indexOf('\n') > -1) {
      NBSDKS_SHARECODEs = process.env.NBSDKS_SHARECODE.split('\n');
  } else {
      NBSDKS_SHARECODEs = process.env.NBSDKS_SHARECODE.split()
  };
  Object.keys(NBSDKS_SHARECODEs).forEach((item) => {
        if (NBSDKS_SHARECODEs[item]) {
          NBSDKS_SHARECODEArr.push(NBSDKS_SHARECODEs[item])
        }
      })
} else {
   NBSDKS_SHARECODEArr.push($.getdata('NBSDKS_SHARECODE'));
   $.getjson('NBSDKS_SHARECODE', []).forEach(sharecode => NBSDKS_SHARECODEArr.push(sharecode));
}
//获取水滴助力码结束
