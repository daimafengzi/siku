...
//let VAL_signcookie = "1&_device=iPhone&57705536-AA77-4A01-A630-9A4D462669E4&2.3.10; 1&_token=318612638&8A4A68D0140C2670DADE6CF597D16A333F9CD0C5678C73809059CA0449103524ACFE3231FE3E20M7676ADE436AD960_; NSUP=; XD=mM/H065qlpooAmCIYhzNt+Wp9YTRC5+SnDxUkkCEZoKJOelrmzKOt/aJosXvnOxVo10Imjb2gpu/tI/vPAM+aQ==; XUM=57705536-AA77-4A01-A630-9A4D462669E4; ainr=0; boot_mark=1637867485.807793; c-oper=%E6%9C%AA%E7%9F%A5; channel=ios-b1; device_model=iPhone X; displayMode=0; idfa=8FEF3DD0-AF87-4090-8131-BD85F6060405; idfaLimit=1; impl=com.ximalaya.tingLite; ip=10.143.254.63; net-mode=WIFI; res=1125%2C2436; update_mark=1581094806.475319416; _xmLog=h5&07f05c4a-7668-40a0-b497-0212b152b167&2.1.2"
const VAL_signcookie = $.getdata("XMLY_SPEED_COOKIE")
$.signinfo = {}
  ...
  return new Promise((resolve, reject) => {
    ...
    $.post(url, (error, response, data) => {
      try {
        ...
        $.msg($.name, `普通签到结果: 失败`, `说明: ${e}`)
        $.log(`❌ ${$.name} signapp - 普通签到失败: ${e}`)
...
function shoutingjb(){
	return new Promise((resolve, reject) => {
    const url = {
      url: `https://m.ximalaya.com/speed/task-center/check-in/record?time=${time}`,
      headers: { Cookie: VAL_signcookie }
    }
    url.headers['Accept'] = 'application/json, text/plain, */*'
    url.headers['Accept-Encoding'] = 'gzip, deflate, br'
    url.headers['Accept-Language'] = 'zh-cn'
    url.headers['Connection'] = 'keep-alive'
    url.headers['Host'] = 'mwsa.ximalaya.com'
	url.headers['Referer'] = 'https://m.ximalaya.com/growth-ssr-speed-welfare-center/page/welfare'
    url.headers['User-Agent'] =
      'ting_v2.3.10_c5(CFNetwork, iOS 15.1, iPhone10,3) ;xmly(lite)/2.3.10/ios_1'
    $.get(url, (error, response, data) => {
      try {
        data = JSON.parse(data);
		//console.log(data);//打印出需要提现详情--完全。
		console.log(data.historyDays);//打印出需要提现详情--完全。
		console.log(data.isTickedToday);//打印出需要提现详情--完全。
      } catch (e) {
        $.msg($.name, `提现签到结果: 失败`, `说明: ${e}`)
        $.log(`❌ ${$.name} signapp - 提现签到: ${e}`)
        resolve()
      }
    })
