...
const jsname = '永川头条'
const $ = Env(jsname)
const logDebug = 0 //调试开关，改成1就打印全部包
...
let httpResult //用来放http的返回，全局变量作为缓存

//环境变量
//青龙是node环境，会读取process.env.ycttToken，也就是环境变量
//V2X正常跑脚本是非node环境，会调用$.getdata()接口获取变量
//let userCookie = ($.isNode() ? process.env.ycttToken : $.getdata('ycttToken')) || '';

//本地变量
let userCookie = 'acw_tc=2f624a0916421607631482802e14f0cadb0d9f0c8cc48a709b6ac7f3ada08c'
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        //将userCookieArr里面每个元素取出来，放到token里去遍历
        for(let token of userCookieArr) {
            
            //sessionId取长度为32的随机字符串
            //跟要写的app有关，为什么这样做，自己想
            let sessionId = randomString(32)
            
            //await: 等待这个函数结束之后才会进行下一个命令
            await getInfo(sessionId,token)
        }
    }
})()
.catch((e) => $.logErr(e))
...
async function getInfo(sessionId,token) {
    //打印调用的函数，调试用
    let caller = printCaller()
    
    //url：发包的链接
    let url = `https://api.cqliving.com/point/myInfo.html`
    
    //最短的请求头，自己不断尝试删掉部分header重放后得出
    let header = {
        'Host': 'api.cqliving.com',
        'Content-Type': 'application/x-www-form-urlencoded'
    }
    
    //请求体：post/put方法才有，get方法没有
    let body = `appId=9&sessionId=${sessionId}&token=${token}&v=1`
    
    //组成一个请求
    let urlObject = {
        url: url,
        headers: header,
        body: body, //get请求没有body，把这行删掉
    }
    
    //调试用，打印完整的发包，调试完注释掉保持日志美观
    ...
    //调试用，打印完整result，调试完注释掉保持日志美观
    //console.log(result)
    
    //根据返回内容，决定后续做什么任务
    if(result.code == 0) {
        let signStr = (result.data.sign) ? '已签到' : '未签到'
        //反引号``里面的${}可以把变量打印出来
        console.log(`现有积分：${result.data.currentPoint}`)
        console.log(`签到状态：${signStr}`)
    } else {
