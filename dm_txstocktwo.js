...
APP捉包把整个URL放进去TxStockAAppUrl，header转成JSON字符串之后放进去TxStockAAppHeader
微信微证券公众号捉包后header转成JSON字符串之后放进去TxStockWWxHeader
export TxStockAAppUrl='https://wzq.ten....#https://wzq.ten....#https://wzq.ten....'
export TxStockAAppHeader='{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}'
export TxStockWWxHeader='{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}#{"Host":"...","Accept":"...",...}'

重写食用
TxStockAAppUrl与TxStockAAppHeader：打开APP，点击头像->右上角金币->获取金币
TxStockWWxHeader：打开 腾讯自选股微信版|微证券 公众号，右下角好福利->福利中心
...
var TxStockAAppUrl
var TxStockAAppHeader
var TxStockWWxHeader
    ...
    if($request.url.indexOf("activity_task_daily.fcgi?") > -1 ||
       $request.url.indexOf("activity_task_continue.fcgi?") > -1) {
        if($request.url.indexOf("openid=") > -1)
        {
            //APP包
            let msgStr = ''
            $.setdata($request.url,'TxStockAAppUrl')
            $.log(`获取TxStockAAppUrl成功: ${$request.url}\n`)
            msgStr += `获取TxStockAAppUrl成功: ${$request.url}\n`
            $.setdata(JSON.stringify($request.headers),'TxStockAAppHeader')
            $.log(`获取TxStockAAppHeader成功: ${JSON.stringify($request.headers)}\n`)
            msgStr += `获取TxStockAAppHeader成功: ${JSON.stringify($request.headers)}\n`
        ...
        {
            //微信包
            $.setdata(JSON.stringify($request.headers),'TxStockWWxHeader')
            $.log(`获取TxStockWWxHeader成功: ${JSON.stringify($request.headers)}\n`)
            $.msg(`获取TxStockAAppHeader成功: ${JSON.stringify($request.headers)}\n`)
        }
    }
}
...
async function checkEnv()
{
    if($.isNode())
    {
        TxStockAAppUrl = process.env.TxStockAAppUrl
        TxStockAAppHeader = process.env.TxStockAAppHeader
        TxStockWWxHeader = process.env.TxStockWWxHeader
    }
    else
    {
        TxStockAAppUrl = $.getdata('TxStockAAppUrl')
        TxStockAAppHeader = $.getdata('TxStockAAppHeader')
        TxStockWWxHeader = $.getdata('TxStockWWxHeader')
    }
    
    if(!TxStockAAppUrl || !TxStockAAppHeader || !TxStockWWxHeader)
    {
        str1 = TxStockAAppUrl ? "" : "TxStockAAppUrl"
        str2 = TxStockAAppHeader ? "" : "TxStockAAppHeader"
        str3 = TxStockWWxHeader ? "" : "TxStockWWxHeader"
        $.log(`未找到环境变量: ${str1} ${str2} ${str3}\n`)
        return false
    }
    
    if (TxStockAAppUrl.indexOf('#') > -1) {
        appUrlArrs = TxStockAAppUrl.split('#');
        console.log(`您选择的是用"#"隔开TxStockAAppUrl\n`)
    } else if (TxStockAAppUrl.indexOf('\n') > -1) {
        appUrlArrs = TxStockAAppUrl.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockAAppUrl\n`)
    } else {
        appUrlArrs = [TxStockAAppUrl]
    };
    ...
    if (TxStockAAppHeader.indexOf('#') > -1) {
        appHeaderArrs = TxStockAAppHeader.split('#');
        console.log(`您选择的是用"#"隔开TxStockAAppHeader\n`)
    } else if (TxStockAAppHeader.indexOf('\n') > -1) {
        appHeaderArrs = TxStockAAppHeader.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockAAppHeader\n`)
    } else {
        appHeaderArrs = [TxStockAAppHeader]
    };
    ...
    if (TxStockWWxHeader.indexOf('#') > -1) {
        wxHeaderArrs = TxStockWWxHeader.split('#');
        console.log(`您选择的是用"#"隔开TxStockWWxHeader\n`)
    } else if (TxStockWWxHeader.indexOf('\n') > -1) {
        wxHeaderArrs = TxStockWWxHeader.split('\n');
        console.log(`您选择的是用"\\n"隔开TxStockWWxHeader\n`)
    } else {
        wxHeaderArrs = [TxStockWWxHeader]
    };
