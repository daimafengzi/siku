...
循环助力
cookie变量KS_COOKIE
助力ID变量KSZZZJB_SHARECODE
[Script]
cron "3 0 * * *" script-path=https://github.com/daimafengzi/siku.git, tag=快手极速版周周赚金币
*/
const $ = new Env('快手极速版周周赚金币')
let cookieArr = [];
let KSZZZJB_SHARECODEArr = [], message, allMessage = '';
if ($.isNode()) {
...
//获取助力ID
if ($.isNode()) {
  if (process.env.KSZZZJB_SHARECODE && process.env.KSZZZJB_SHARECODE.indexOf('\n') > -1) {
      KSZZZJB_SHARECODEs = process.env.KSZZZJB_SHARECODE.split('\n');
  } else {
      KSZZZJB_SHARECODEs = process.env.KSZZZJB_SHARECODE.split()
  };
  Object.keys(KSZZZJB_SHARECODEs).forEach((item) => {
        if (KSZZZJB_SHARECODEs[item]) {
          KSZZZJB_SHARECODEArr.push(KSZZZJB_SHARECODEs[item])
        }
      })
} else {
   KSZZZJB_SHARECODEArr.push($.getdata('KSZZZJB_SHARECODE'));
   $.getjson('KSZZZJB_SHARECODE', []).forEach(sharecode => KSZZZJB_SHARECODEArr.push(sharecode));
}
//获取助力ID结束
 ...
 if(!cookieArr[0]){
  ...
  //账号循环开始
		  for (let i = 0; i < cookieArr.length; i++) {
			...
			//化肥助力body获取开始
			  for (let ix = 0; ix < KSZZZJB_SHARECODEArr.length; ix++) {
				  if (KSZZZJB_SHARECODEArr[ix]) {
					 KSZZZJB_SHARECODEVal = KSZZZJB_SHARECODEArr[ix]; 
					 console.log(`-----------助力ID获取成功----------------`)
					 console.log(KSZZZJB_SHARECODEVal)
					 console.log(`------------下面开始快手极速版周周赚金币助力---------------`)
					 await officialSign();
				  }
			  }
			//化肥助力body获取结束
