...
================Loon==============
[Script]
cron "55 59 9,11,14,16,19 * * *" script-path=https://gitee.com/lxk0301/jd_scripts/raw/master/jd_car_exchange.js, tag=微信京东9.9-9.8优惠券

 */
const $ = new Env('微信京东9.9-9.8优惠券');
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            console.log(`【微信京东9.9-9.8优惠券抢券结果】${JSON.stringify(data.subCodeMsg)}\n`);
			//自行添加通知项目开始
			var str="{"+data.subCodeMsg+"}";
			if(str.indexOf('领取成功') !=-1){
				//notify.sendNotify($.name, `京东账号  ${$.nickName || $.UserName}\n【微信京东9.9-9.8优惠券抢券结果】${JSON.stringify(data.subCodeMsg)}`);
				}
			//自行添加通知项目结束
          }
        }
