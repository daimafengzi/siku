...
助力码变量NBKS_SHARECODE
[Script]
cron "30 0 * * *" script-path=https://github.com/daimafengzi/siku.git, tag=快手内部助力
*/
const $ = new Env('快手内部助力')
let cookieArr = [];
let NBKS_SHARECODEArr = [], message, allMessage = '';
if ($.isNode()) {
...
//获取助力码
if ($.isNode()) {
  if (process.env.NBKS_SHARECODE && process.env.NBKS_SHARECODE.indexOf('\n') > -1) {
      NBKS_SHARECODEs = process.env.NBKS_SHARECODE.split('\n');
  } else {
      NBKS_SHARECODEs = process.env.NBKS_SHARECODE.split()
  };
  Object.keys(NBKS_SHARECODEs).forEach((item) => {
        if (NBKS_SHARECODEs[item]) {
          NBKS_SHARECODEArr.push(NBKS_SHARECODEs[item])
        }
      })
} else {
   NBKS_SHARECODEArr.push($.getdata('NBKS_SHARECODE'));
   $.getjson('NBKS_SHARECODE', []).forEach(sharecode => NBKS_SHARECODEArr.push(sharecode));
}
//获取助力码结束
...
!(async() => {
 ...
 if(!cookieArr[0]){
  ...
  //助力body获取开始
  for (let ix = 0; ix < NBKS_SHARECODEArr.length; ix++) {
	  if (NBKS_SHARECODEArr[ix]) {
		 NBKS_SHARECODEVal = NBKS_SHARECODEArr[ix]; 
		 console.log(`-----------助力码获取成功----------------`)
		 console.log(NBKS_SHARECODEVal)
		//账号循环开始
