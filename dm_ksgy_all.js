...
水滴助力码变量KS_SHARECODE
[Script]
cron "5 0 * * *" script-path=https://github.com/daimafengzi/siku.git, tag=快手内部助力all
*/
const $ = new Env('快手内部助力all')
let cookieArr = [];
...
function  officialSignhf() {
   return new Promise((resolve, reject) => {
	 let signurl = {
        ...
        body: '{"fid":"'+fid+'","cc":"share_copylink","followRefer":"151","shareMethod":"TOKEN","sharePosition":"MANURE_AID_INVITE_BUTTON","kpn":"NEBULA","subBiz":"OD_MANURE_SHARE","shareId":"16833683147929","source":"PASSPHRASE_BACK","shareMode":"APP","originShareId":"16833683147929","enableWK":"1","layoutType":"4","shareObjectId":"'+shareObjectId+'","shareUrlOpened":"0","hyId":"orchard","timestamp":"'+Date.now()+'"}'
   }
    $.post(signurl, (error, response, data) => {
		//console.log("化肥助力");
		data = JSON.parse(data);
...
function  officialtaskCenter() {
   return new Promise((resolve, reject) => {
	 let signurl = {
		url: 'https://ug-fission.kuaishou.com/rest/n/darwin/orchard/overview',
	    headers: {Cookie: cookieVal.replace('kpn=NEBULA', 'kpn=KUAISHOU'),'Content-Type': 'application/json;charset=utf-8'},
        body: '{"fid":"'+fid+'","cc":"share_copylink","followRefer":"151","shareMethod":"TOKEN","sharePosition":"WATER_AID_INVITE_BUTTON","kpn":"KUAISHOU","subBiz":"OD_WATER_SHARE","source":"PASSPHRASE_BACK","shareMode":"APP","enableWK":"1","layoutType":"4","shareObjectId":"'+shareObjectId+'","shareUrlOpened":"0","hyId":"orchard","timestamp":"'+Date.now()+'"}'
   }
    $.post(signurl, (error, response, data) => {
		data = JSON.parse(data);
		console.log(data.data.popupEventList);
